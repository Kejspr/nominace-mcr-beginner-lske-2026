...
from config import AGGREGATED_XML, EXCEL_CSV, PRESENTATION_HTML, QUALIFYING_PLACES
...
TABLE_HEAD_POSTUP = f"<th>{LEGEND_LINK_KRAJE}</th><th>{LEGEND_LINK_POSTUPUJE}</th>"
    ...
    for category_name in sorted_categories:
        ...
        for row in category_rows:
            ...
            rows_html.append(
                ...
                f"<td class='{postup_class(postup_z_kraje, 'postup-z-kraje')}'>"
                f"{escape_html(postup_z_kraje)}</td>"
                f"<td class='{postup_class(postupuje, 'postupuje')}'>"
